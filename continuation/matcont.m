...
%    addpath([cd '/Help/']);
    ...
    if ~(exist('BVP_BPC_jacC.mexsol','file') || exist('BVP_BPC_jacC.mexglx','file') || exist('BVP_BPC_jacC.mexa64','file') || ...
            exist('BVP_BPC_jacC.mexmac','file') || exist('BVP_BPC_jacC.mexw32','file') || exist('BVP_BPC_jacC.mexw64','file'))
        mex -O BVP_LC_jac.c;
        mex -O BVP_PD_jac.c;
    ...
    %case yes:load the last used system
    if (exist(session,'file')==2) 
        ...
        if isempty(FigPos) 
            FigPos=[];FigPos.mainwindow=[];FigPos.starter=[90.0 3 50 18];FigPos.continuer=[150 2.995 48.167 31.438];
...
calcultation_progress = 1;
start_cont;
calculation_progress = 0;

%-----------------------------------------------------------------
function varargout = backward_callback(h,eventdata,handles,varargin)
...
%-----------------------------------------------------------------
function varargout = extend_callback(h,eventdata,handles,varargin)
...
%------------------------------------------------------------------
function varargout = point_callback(handles)
global gds MC;
tag = get(gcbo,'Tag');
...
%------------------------------------------------------------------
function varargout = type_callback(varargin)
global gds
newtag = get(gcbo,'Tag');
...
i=1;        
while (exist(file)~=0)&(i<sys.gui.ncurves)
      d(i) = dir(file);
      i = i+1;
...
if (i==sys.gui.ncurves)&(exist(file)~=0)
    d(i) = dir(file);
    da   = datenum(vertcat(d(:).date));
    j = find(da==min(da));
    j = j(1);
...
%------------------------------------------------------------------
function varargout = start_cont(varargin)
