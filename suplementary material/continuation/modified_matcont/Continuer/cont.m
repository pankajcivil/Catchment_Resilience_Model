...
%     end
% else 
    MC.D2 = [];MC.D3 = [];MC.PRC=[];MC.dPRC=[];MC.numeric_fig = []; 
...
%     options = feval(cds.curve, 'options');
    ...
    % TJP: Add odeset to cds.options
    %cds.options.odeset = opt.odeset;
    cds.options.MaxSecantIters = contget(cds.options, 'MaxSecantIters', 2);
    ...
    %
    cds.h_inc_fac = 1.25;  %TJP: change h_inc_fac from 1.3333 to 1.25
    cds.h_dec_fac = 0.5;  
    cds.dir_check_step = 3;
    cds.dir_check_angle = 0.99; %TJP: increased angle from 0.9 to 0.99
...
% TJP: store original number of secant steps
originalMaxSecantIters = cds.options.MaxSecantIters;
originalMaxCorrIters = cds.options.MaxCorrIters;

% TJP Modification : make use of additional cds.options field to restrict
% min mand max param values. Has not been tested for >1 param
% TJP also removed restriction on npoints
display(['....At i=', num2str(1),', param= ', num2str(x1(end,1),6)]);  
%while i < npoints && x1(end,end) > cds.options.ParamMin && x1(end,end) < cds.options.ParamMax
while x1(end,end) > cds.options.ParamMin && x1(end,end) < cds.options.ParamMax
    % correction
    %
    corrections = 1;
    while 1
      ...
      if ~isempty(x2)  && (i < cds.dir_check_step  || (v1'*v2 > cds.dir_check_angle))
            ...
            if ~failed
                % TJP: restore original number of newton secant steps and
                % originalMaxCorrIters
                if corrections==1 && cds.h > 2*cds.h_min;
                    cds.options.MaxSecantIters = originalMaxSecantIters; 
                    cds.options.MaxCorrIters = originalMaxCorrIters; 
                end
      ...
      elseif ~isempty(x2)  && v1'*v2 <= cds.dir_check_angle
          display(['... The angle between to current and prior directions vectors is ' ,num2str( acos(v1'*v2)), ' and the max allowed angle is ',num2str(acos(cds.dir_check_angle ) ) ]);
          display( '    The step size will therefore be reduced.' );
      end
      ...
      if cds.h > cds.h_min
          ...
          % TJP: reduce maximum newton secant iterations  
          cds.options.MaxSecantIters = 1;
          %cds.options.MaxCorrIters = 2*originalMaxCorrIters;
...
%% Misc clean ups  
  ...
  if mod(i,1)==0;
      ...
      global lds eds
      if ~isempty(lds)
          ups = reshape(x2(1:end-2,:),lds.nphase,lds.tps);      
          display(['....At i=', num2str(i),', p=', num2str(x2(end,1),6), ...
              ' , Min & Max 1st LC= ', num2str(min(ups(1,:)),6),', ', num2str(max(ups(1,:)),6), ...
              ' , Min & Max 2nd LC= ', num2str(min(ups(2,:)),6),', ', num2str(max(ups(2,:)),6), ...
              ' , step size= ', num2str(cds.h),' of ',num2str(cds.h_max), ...
              ' , corr= ', num2str(corrections),' , its= ',num2str(it) ]);  
      else 
          display(['....At i=', num2str(i),', p=', num2str(x2(end,1),6), ...
              ' , State variables 1 & 2 = ', num2str(x2(1,1),6),', ', num2str(x2(2,1),6), ...              
              ' , step size= ', num2str(cds.h),' of ',num2str(cds.h_max), ...
              ' , corr= ', num2str(corrections),' , its= ',num2str(it) ]);           
      end
  end
  % TJP change - number of iterations, it, for h correction increased from <4 to <=cds.options.MaxCorrIters - 2.  
  if cds.h < cds.h_max & corrections==1 & it <= cds.options.MaxCorrIters-2
...
%  fprintf('%d/%d \r', i, cds.ndim);
  display('...Corecting Starting Point: Fist attempt failed, changing vector index');
  v0(i) = 1;
