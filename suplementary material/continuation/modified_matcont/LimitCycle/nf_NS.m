function result = nscoefficient(x0)
%
% calculates NS normal form coefficient
%
global lds 
[xt,p,T] = rearr(x0);pt = n2c(p);
%computing theta
...
for jk=lds.tsts
    ...
    for c=lds.cols                  
        ...
        meshindex= lds.ncol*(jk-1)+c;    
        mesh_time = lds.finemsh(meshindex);
        
        xt = xp(:,range6(c));
        sten = zeros(lds.nphase,lds.nphase);
        wtk = lds.wt(kr1,c(ones(1,lds.nphase)))';
        sjac  = odejac(mesh_time, xt,pt);
        hess = odehess(mesh_time,xt,pt);
        tens = odetens(mesh_time,xt,pt);
        f1(range2) = feval(lds.func, mesh_time,  xt, pt{:});
...
result =conj(alpha)*(1/2*dot(w1,(1/T)*(fxtens.')+(fxhessh11.')+(fxhessh20.'))-(a/T)*dot(w1,fxjac*v1))+abs(alpha)^2*(1i*a*theta)/(T^2);
% ---------------------------------------------------------------
