...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    mwIndex ntst, ncol, nphase, *ActiveParams, ncoords, nfreep, tps;
    ...
    mwIndex filled, elementcounter, remm;
    mwIndex i,j,k,l,l2;
    
	/*int *range1, *range2, *range3, *range4;*/
    mwIndex *range1, *range2, *range3, *range4;
    ...
    ActiveParams = mxCalloc(nfreep,sizeof(mwIndex));
    frhstmp = mxGetPr(thisfield);
    for (i=0; i<nfreep; i++) 
        *(ActiveParams+i) = (mwIndex)(*(frhstmp+i));         
	...
	*jc = 0;
	...
	/* ------------------------ */    
	range1 = mxCalloc(ncol+1,sizeof(mwIndex));
	range2 = mxCalloc(ncol*nphase,sizeof(mwIndex));
	range3 = mxCalloc((ncol+1)*nphase,sizeof(mwIndex));
	range4 = mxCalloc(nphase,sizeof(mwIndex));
	...
	for (i=0; i<ntst; i++) {
		...
		for (j=0; j<ncol; j++) {
            ...
            /* TJP: Calculate time point of collocation points */
            *meshtimeEval=0;
            for (k=0; k<(ncol+1); k++) {
                *meshtimeEval = *meshtimeEval + (*(finemesh + i*ncol + k)) * (*(wt+j*(ncol+1)+k));
            }
            ...
            *meshtimeJac = *meshtimeEval; 
            ...
            /* TJP: increment nrhs from 2 to 3 */  
			mexCallMATLAB(1,jaclhs,3,jacrhs,"odejacp");
			frhstmp = mxGetPr(jaclhs[0]);            
            
            for (k=0; k<nfreep; k++)
                for (l=0; l<nphase; l++)
                    *(jacp+k*nphase+l) = *(frhstmp+(*(ActiveParams+k)-1)*nphase+l);
            mxDestroyArray(jaclhs[0]);
